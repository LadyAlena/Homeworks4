...
void sortAddress(Address* address, int countAdresses) {
	
	Address buferAddress;

	bool swap;

	do {
		swap = false;
		for (int i = 0; i < countAdresses - 1; ++i) {

			if (address[i].GetNameTown() > address[i + 1].GetNameTown()) {

				buferAddress = address[i+1];
				address[i+1] = address[i];
				address[i] = buferAddress;
...
int main(int argc, char** argv) {
	...
	sortAddress(ptrAddress, countAddress);
